...
import { revertEmailChangeApi } from "@/lib/api/users"
    ...
    <div className="bg-slate-50 flex min-h-svh flex-col items-center justify-center gap-6 p-6 md:p-10">
      <div className="flex w-full max-w-sm flex-col gap-6">
        ...
        <div className="bg-white rounded-lg border p-8 text-center">
          {status === 'loading' && (
            <div>
              <h2 className="text-xl font-semibold text-blue-950 mb-2">正在撤销邮箱变更...</h2>
              <p className="text-sm text-gray-600">请稍候</p>
            </div>
          )}
          {status === 'success' && (
            <div>
              <h2 className="text-xl font-semibold text-green-700 mb-2">邮箱已恢复</h2>
          ...
          {status === 'error' && (
            <div>
              <h2 className="text-xl font-semibold text-red-700 mb-2">撤销失败</h2>
              <p className="text-sm text-gray-600 mb-4">{message}</p>
              <Link href="/login" className="text-blue-600 hover:underline text-sm">
                返回登录
              </Link>
